...
public static async Task TerminateAccount(ServerSettings settings, MySqlConnection connection, string username)
{
    try
    {
        ...
        }))
        {
            ...
            {
                // sendEmail: false — this is an account termination, not a single-article
                // removal, so we skip the per-article "story removed" notice; the account
                // termination notice below (type 6) already covers this.
                await EventProcessor.HandleRemoveArticle(connection, client, toDelete, username, settings, sendEmail: false);
            }
